...
### Testblock Parkplätze
import osmnx as ox

log("Lade Parkplätze aus OSM...")
try:
    parkplaetze = ox.features_from_polygon(
        hafen.to_crs(4326).union_all(),
        tags={"amenity": "parking"}
    )
    # Nur Polygone (keine Punkte)
    parkplaetze = parkplaetze[parkplaetze.geometry.geom_type.isin(['Polygon', 'MultiPolygon'])]
    parkplaetze = parkplaetze.to_crs(25832)
    log(f"Parkplätze gefunden: {len(parkplaetze)}")
    log(f"Spalten: {list(parkplaetze.columns)}")
except Exception as e:
    log(f"OSM-Fehler: {e}")
    parkplaetze = None
###  Testblock Parkplätze END
